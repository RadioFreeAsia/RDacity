...
#ifdef USE_LIBRIVENDELL

      c->AddSeparator();

      c->AddItem(wxT("BrowseRivendell"),_("Browse R&ivendell library"),FN(OnBrowseRivendellLibrary));

      c->AddItem(wxT("ExportRivendell"),  _("Export to Rivendell"),  FN(OnExportRivendellMix),
                      AudioIONotBusyFlag | WaveTracksExistFlag, 
					  AudioIONotBusyFlag | WaveTracksExistFlag);

      c->AddItem(wxT("ExportRivendellSel"),_("Export Selection to Rivendell"),FN(OnExportRivendellSelection),
                      AudioIONotBusyFlag |TimeSelectedFlag | WaveTracksSelectedFlag,
                      AudioIONotBusyFlag |TimeSelectedFlag | WaveTracksSelectedFlag);

      c->AddSeparator();

#endif /* USE_LIBRIVENDELL */
   /////////////////////////////////////////////////////////////////////////////
...
/**
 * Initialize the Rivendell database connection.  Check that a valid database
 * version is connected to.
 *
 * @param _Db with a pointer to a MYSQL structure.
 * @return bool with true if successful.
 */
bool AudacityProject::InitRivendellDatabase(MYSQL *_Db)
{
   char login[50];
   char password[50];
   char host[50];
   ...
   if (!RivendellCfg->ParseString("mySQL", "Password", password)) {
      wxMessageBox(_("Unable to find password in rd configuration"), _("Rivendell mySQL"), wxICON_ERROR|wxOK);
      return false;
   }
   if (!RivendellCfg->ParseString("mySQL", "Hostname", host)) {
...
/**
 * Event handler for browsing the Rivendell library.
 *
 * @return void
 */
void AudacityProject::OnBrowseRivendellLibrary()
{
   // Local variables.
   MYSQL m;
   wxString directorySeparator;
   char  soundsDir[255]; // FIXME: size
   wxString hold_soundsDir;
   int   cartId;
   int   cutId;
   wxString filename;
   wxString selection;
...
void AudacityProject::OnExportRivendellMix()
{
   MYSQL		m;
   Exporter e;
   wxString query;
   ...
   if (!e.JustInitValidate(this, false, 0.0, mTracks->GetEndTime()))
       return;
/****************************************************************************/
//  This is to check for data base date BEFORE we start constructing
//  the dialog(s).  Easier to do it now that later.    Todd Baker 10/10/13
/***************************************************************************/
   ...
   RivendellDialog dlog(this, &m, false);
   dlog.ShowModal();
 
   mysql_close(&m);
}

void AudacityProject::OnExportRivendellSelection()
{
   MYSQL		m;
   Exporter e;
   wxString query;
   ...
   if (!e.JustInitValidate(this, true, 0.0, mTracks->GetEndTime()))
       return;
...
/***************************************************************************/
   ...
   RivendellDialog dlog(this, &m, true);
   dlog.ShowModal();
